...
__global__ void calculate_velocity(t_particles p_in, t_particles p_out, int N, float dt){
  ...
  for ( int batch = 0; batch < (N - 1) / DEFAULT_SHARED_LENGTH + 1; batch++ ){
    ...
    if ( idx_in_blk < DEFAULT_SHARED_LENGTH ){
...
__global__ void centerOfMass(t_particles p, float* comX, float* comY, float* comZ, float* comW, int* lock, const int N) {
  ...
  const int thr_in_block = blockDim.x / 32;

  extern __shared__ float cache[];
  float* cache_x = (float*)cache;
  float* cache_y = (float*)&cache[thr_in_block];
  float* cache_z = (float*)&cache[thr_in_block*2];
  float* cache_w = (float*)&cache[thr_in_block*3];
  ...
  float4 com = {0, 0, 0, 0};
  while ( idx < N ) {
    const float dx = p.pos_x[idx] - com.x;
    const float dy = p.pos_y[idx] - com.y;
    const float dz = p.pos_z[idx] - com.z;

    const float dw = ((p.weight[idx] + com.w) > 0.0f) ? ( p.weight[idx]  / (p.weight[idx] + com.w)) : 0.0f;
  ...
  for ( int stride = 16; stride > 0; stride = stride >> 1 ) {
    const float dx = __shfl_down_sync(0xffffffff, com.x, stride) - com.x;
    const float dy = __shfl_down_sync(0xffffffff, com.y, stride) - com.y;
    const float dz = __shfl_down_sync(0xffffffff, com.z, stride) - com.z;
    
    const float tmp_w = __shfl_down_sync(0xffffffff, com.w, stride);
    const float dw = ((tmp_w + com.w) > 0.0f) ? ( tmp_w  / (tmp_w + com.w)) : 0.0f;
  ...
  // only first threads in the warps continue
  if ( (threadIdx.x & 31) == 0 ) {
    const int cache_idx_32 = threadIdx.x / 32;

    cache_x[cache_idx_32] = com.x;
    cache_y[cache_idx_32] = com.y;
    cache_z[cache_idx_32] = com.z;
    cache_w[cache_idx_32] = com.w;
    __syncthreads();

    // Phase 2
    for ( int stride = blockDim.x / 64; stride > 0; stride >>= 1 ) {

      if (cache_idx_32 < stride){
        const float dx = cache_x[cache_idx_32 + stride] - cache_x[cache_idx_32]; 
        const float dy = cache_y[cache_idx_32 + stride] - cache_y[cache_idx_32]; 
        const float dz = cache_z[cache_idx_32 + stride] - cache_z[cache_idx_32]; 
      
        const float dw = ((cache_w[cache_idx_32 + stride] + cache_w[cache_idx_32]) > 0.0f) ?
                            ( cache_w[cache_idx_32 + stride]  / (cache_w[cache_idx_32 + stride] + cache_w[cache_idx_32])) : 0.0f;
      
        cache_x[cache_idx_32] += dx * dw;
        cache_y[cache_idx_32] += dy * dw;
        cache_z[cache_idx_32] += dz * dw;
        cache_w[cache_idx_32] += cache_w[cache_idx_32 + stride];
      }
      __syncthreads();
    }
  }
  ...
  if ( threadIdx.x == 0 ) {
    ...
    while (0 != (atomicCAS(lock, 0, 1))) {}
      ...
      const float dx = cache_x[0] - comX[0]; 
      const float dy = cache_y[0] - comY[0]; 
      const float dz = cache_z[0] - comZ[0];

      const float dw = ((cache_w[0] + comW[0]) > 0.0f) ? ( cache_w[0]  / (cache_w[0] + comW[0])) : 0.0f;
