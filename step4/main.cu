...
int main(int argc, char **argv)
{
  ...
  *lock = 0;
  ...
  // create streams
  cudaStream_t vel_stream, com_stream;
  ...
  for(int s = 0; s < steps; s++) {
    ...
    // launch calculate_velocity kernel and COM kernel with same input
    calculate_velocity<<<gridDim, blockDim, 0, vel_stream>>>(particles_gpu_in, particles_gpu_out, N, dt);
    centerOfMass<<<gridDimRed, blockDimRed, (red_thr_blc * sizeof(float) * 4) / 32, com_stream>>>
    ...
    if (writeFreq > 0 && (s % writeFreq == 0)) {
      // record event at the end of com_stream so copy_stream will wait for data
      cudaEvent_t com_event;
      cudaEventCreate(&com_event);
      cudaEventRecord(com_event, com_stream);

      //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      //                          FILL IN: synchronization and file access logic (step 4)                             //
      //////////////////////////////////////////////////////////////////////////////////////////////////////////////////

      // async copy input of the n-th iteration
      cudaMemcpyAsync(particles_cpu.pos_x, particles_gpu_in.pos_x, N * sizeof(float), cudaMemcpyDeviceToHost, copy_stream);
      cudaMemcpyAsync(particles_cpu.pos_y, particles_gpu_in.pos_y, N * sizeof(float), cudaMemcpyDeviceToHost, copy_stream);
      cudaMemcpyAsync(particles_cpu.pos_z, particles_gpu_in.pos_z, N * sizeof(float), cudaMemcpyDeviceToHost, copy_stream);

      cudaMemcpyAsync(particles_cpu.vel_x, particles_gpu_in.vel_x, N * sizeof(float), cudaMemcpyDeviceToHost, copy_stream);
      cudaMemcpyAsync(particles_cpu.vel_y, particles_gpu_in.vel_y, N * sizeof(float), cudaMemcpyDeviceToHost, copy_stream);
      cudaMemcpyAsync(particles_cpu.vel_z, particles_gpu_in.vel_z, N * sizeof(float), cudaMemcpyDeviceToHost, copy_stream);
      // record event in the copy_stream when particle data copy is done, so CPU will be able to save that data
      cudaEvent_t particle_event;
      cudaEventCreate(&particle_event);
      cudaEventRecord(particle_event, copy_stream);

      
      // async copy COM data when com_stream finishes(with event)
      cudaStreamWaitEvent(copy_stream, com_event, 0);
      cudaMemcpyAsync(&comOnGPU.x, com_x, sizeof(float), cudaMemcpyDeviceToHost, copy_stream);
      cudaMemcpyAsync(&comOnGPU.y, com_y, sizeof(float), cudaMemcpyDeviceToHost, copy_stream);
      cudaMemcpyAsync(&comOnGPU.z, com_z, sizeof(float), cudaMemcpyDeviceToHost, copy_stream);
      cudaMemcpyAsync(&comOnGPU.w, com_w, sizeof(float), cudaMemcpyDeviceToHost, copy_stream);

      // save particle data when copy_stream reaches particle_event event
      cudaEventSynchronize(particle_event);
      h5Helper.writeParticleData(record);

      // save particle data when copy_stream reaches com_done event
