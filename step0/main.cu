...
int main(int argc, char **argv)
{
  ...
  */
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  cudaMalloc<float>(&particles_gpu.pos_x, N * sizeof(float));
  cudaMalloc<float>(&particles_gpu.pos_y, N * sizeof(float));
  cudaMalloc<float>(&particles_gpu.pos_z, N * sizeof(float));

  cudaMalloc<float>(&particles_gpu.vel_x, N * sizeof(float));
  cudaMalloc<float>(&particles_gpu.vel_y, N * sizeof(float));
  cudaMalloc<float>(&particles_gpu.vel_z, N * sizeof(float));

  cudaMalloc<float>(&particles_gpu.weight, N * sizeof(float));

  cudaMalloc<float>(&velocities_gpu.x, N * sizeof(float));
  cudaMalloc<float>(&velocities_gpu.y, N * sizeof(float));
  cudaMalloc<float>(&velocities_gpu.z, N * sizeof(float));
  ...
  for(int s = 0; s < steps; s++)
  {
    ...
    cudaMemset(velocities_gpu.x, 0, N * sizeof(float));
    cudaMemset(velocities_gpu.y, 0, N * sizeof(float));
    cudaMemset(velocities_gpu.z, 0, N * sizeof(float));

    calculate_gravitation_velocity<<<gridDim, blockDim>>>(particles_gpu, velocities_gpu, N, dt);
    calculate_collision_velocity<<<gridDim, blockDim>>>(particles_gpu, velocities_gpu, N, dt);
    update_particle<<<gridDim, blockDim>>>(particles_gpu, velocities_gpu, N, dt);
