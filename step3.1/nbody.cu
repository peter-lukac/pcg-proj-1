...
{ 
  ...
  float* cache_y = (float*)&cache[blockDim.x];
  float* cache_z = (float*)&cache[blockDim.x*2];
  float* cache_w = (float*)&cache[blockDim.x*3];
  ...
  cache_x[cache_idx] = com.x;
  cache_y[cache_idx] = com.y;
  cache_z[cache_idx] = com.z;
  cache_w[cache_idx] = com.w;
  __syncthreads();
  ...
  for ( int stride = blockDim.x / 2; stride > 0; stride >>= 1 ) {
    ...
    if (cache_idx < stride){
      const float dx = cache_x[cache_idx + stride] - cache_x[cache_idx]; 
      const float dy = cache_y[cache_idx + stride] - cache_y[cache_idx]; 
      const float dz = cache_z[cache_idx + stride] - cache_z[cache_idx]; 
  
      const float dw = ((cache_w[cache_idx + stride] + cache_w[cache_idx]) > 0.0f) ?
                          ( cache_w[cache_idx + stride]  / (cache_w[cache_idx + stride] + cache_w[cache_idx])) : 0.0f;
  
      cache_x[cache_idx] += dx * dw;
      cache_y[cache_idx] += dy * dw;
      cache_z[cache_idx] += dz * dw;
      cache_w[cache_idx] += cache_w[cache_idx + stride];
    }
    __syncthreads();
  }
